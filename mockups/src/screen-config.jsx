...
const Login = () => (
  <div style={{width:'100%',height:'100%',display:'flex',fontFamily:'var(--sans)',background:'var(--paper)'}}>
    ...
    <div style={{flex:1,background:'linear-gradient(140deg, #2e281f 0%, #55382a 60%, #8a5540 100%)',padding:48,display:'flex',flexDirection:'column',justifyContent:'space-between',color:'#faf7f1',position:'relative',overflow:'hidden'}}>
      ...
      <div style={{zIndex:2}}>
        <div className="serif" style={{fontSize:42,fontWeight:500,letterSpacing:-1,lineHeight:1.05,marginBottom:16,maxWidth:440}}>Cierre de caja que respeta tu ritmo.</div>
        <div style={{fontSize:14,opacity:.72,lineHeight:1.55,maxWidth:400}}>Turnos, comisiones, propinas y arqueo — todo en un flujo silencioso, para tablet de frente al mostrador.</div>
      </div>
      {/* decorative arcs */}
    ...
    {/* right form */}
    <div style={{width:440,background:'var(--paper-raised)',padding:56,display:'flex',flexDirection:'column',justifyContent:'center'}}>
      <div style={{fontFamily:'var(--serif)',fontSize:28,fontWeight:500,letterSpacing:-.6,color:'var(--ink-0)',marginBottom:6}}>Bienvenida</div>
      <div style={{fontSize:13,color:'var(--ink-2)',marginBottom:32}}>Ingresa para continuar el turno</div>
      ...
      <Btn variant="primary" size="lg" style={{width:'100%',justifyContent:'center'}}>Ingresar</Btn>
      <div style={{textAlign:'center',fontSize:11,color:'var(--ink-3)',marginTop:22,lineHeight:1.7}}>Demo · admin / encargada / cajera · contraseña <span className="mono">1234</span></div>
    </div>
  </div>
);
